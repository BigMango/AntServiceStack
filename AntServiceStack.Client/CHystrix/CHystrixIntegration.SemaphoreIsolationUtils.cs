    ...
    internal static partial class CHystrixIntegration
    {
        ...
        private static void InitUtilsSemaphoreIsolationHelpMethods(Type utilsSemaphoreIsolationType)
        {
            try
            {
                ...
                {
                    Log.Info("The CHystrix.dll version is low. No required Semaphore Isolation help commands were made.", new Dictionary<string, string>() { { "ErrorCode", "FXD301012" } });
                    return;
                }

                HasCHystrixIsolationUtils = true;
                Log.Info("Successfully inited the CHystrix Semaphore Isolation help methods.");
            }
            catch (Exception ex)
            {
                Log.Info("The CHystrix.dll version is low. Failed to init the CHystrix Semaphore Isolation help methods.", ex, new Dictionary<string, string>() { { "ErrorCode", "FXD301013" } });
            }
        }
