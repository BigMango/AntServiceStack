    ...
    internal static partial class CHystrixIntegration
    {
        ...
        private static void InitInstanceKeyMethods(Type hystrixCommandType, Type utilsSemaphoreIsolationType)
        {
            try
            {
                ...
                {
                    Log.Info("The CHystrix.dll version is low. No required instance key methods ware made.", new Dictionary<string, string>() { { "ErrorCode", "FXD301020" } });
                    return;
                }

                Log.Info("Successfully inited CHystrix instance key methods.");
            }
            catch (Exception ex)
            {
                Log.Info("The CHystrix.dll version is low. IP based circuit breaker will be not used. Use domain based circuit breaker.", ex,
