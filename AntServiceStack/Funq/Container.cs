	...
	public sealed partial class Container : IDisposable
	{
		...
		private ServiceEntry<TService, TFunc> RegisterImpl<TService, TFunc>(string name, TFunc factory)
		{
			if (typeof(TService) == typeof(Container))
				throw new ArgumentException(AntServiceStack.Properties.Resources.Registration_CantRegisterContainer);
        ...
        {
            var errMsg = "Error trying to resolve Service '{0}' or one of its autowired dependencies. Inner Exception: {1}".Fmt(typeof(TService).FullName, ex);
