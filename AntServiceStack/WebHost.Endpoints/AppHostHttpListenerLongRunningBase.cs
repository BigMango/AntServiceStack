...
namespace AntServiceStack.WebHost.Endpoints
{
    public abstract class AppHostHttpListenerLongRunningBase : AppHostHttpListenerBase
    {
        ...
        protected AppHostHttpListenerLongRunningBase(params Assembly[] assembliesWithServices)
            : this(500, assembliesWithServices) { }

        protected AppHostHttpListenerLongRunningBase(int poolSize, params Assembly[] assembliesWithServices)
            : base(assembliesWithServices) { _threadPoolManager = new ThreadPoolManager(poolSize); }
        ...
        private void ListenerCallback(IAsyncResult asyncResult)
        {
            ...
            try
            {
                if (!isListening)
                {
                    _log.Debug(string.Format("Ignoring ListenerCallback() as HttpListener is no longer listening"),
                        new Dictionary<string, string>() 
                        {
                            {"ErrorCode", "FXD300052"}
                        });
            ...
            catch (Exception ex)
            {
                ...
                _log.Warn(errMsg, new Dictionary<string, string>() { { "ErrorCode", "FXD300053" } });
                return;
            }
            finally
            {
            ...
            _log.Info(string.Format("{0} Request : {1}", context.Request.UserHostAddress, context.Request.RawUrl),
                new Dictionary<string, string>() 
                {
                    {"ErrorCode", "FXD300054"}
                });
            ...
            _threadPoolManager.Peek(() =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    _log.Error(error, new Dictionary<string, string>() { { "ErrorCode", "FXD300055" } });
                    ...
                    catch (Exception errorEx)
                    {
                        ...
                        _log.Error(error, new Dictionary<string, string>() { { "ErrorCode", "FXD300056" } });
                    }
                }
