...
namespace AntServiceStack.WebHost.Endpoints.Extensions
{
    ...
    public static class HttpRequestExtensions
    {
        ...
        {
            return GetLastPathInfoFromRawUrl(string.IsNullOrEmpty(request.PathInfo) ? request.RawUrl : request.PathInfo);
        }
        ...
        public static string GetUrlHostName(this HttpRequest request)
        {
            ...
            {
                Log.Error(string.Format("Error trying to get 'Request.Url.Host'", ex),
        ...
        {
            if (request.Url.Segments == null || request.Url.Segments.Length == 0)
                return string.Empty;
        ...
        {
            if (fullPath == null || mappedPathRoot == null)
                return null;
        ...
        public static string GetSupportedContentType(string rawContentType)
        {
            if (rawContentType == null)
                return null;
            rawContentType = rawContentType.Trim().ToLower();

            if (rawContentType.Contains(ContentType.JsonText))
                return ContentType.Json;

            if (rawContentType.Contains(ContentType.XmlText))
                return ContentType.Xml;

            if (rawContentType.Contains(ContentType.JsvText))
                return ContentType.Jsv;

            if (EndpointHost.SupportedFormats == null)
                return null;
            foreach (string contentType in EndpointHost.SupportedFormats.Values)
            {
                if (rawContentType.Contains(contentType))
                    return contentType;
            }
        ...
        public static string GetSupportedContentType(this IHttpRequest httpReq, string responseContentType = null)
        {
            if (httpReq == null)
                return null;

            string requestContentType = httpReq.ContentType == null ? string.Empty : httpReq.ContentType.Trim().ToLower();

            string supportedContentType = GetSupportedContentType(requestContentType);
            if (supportedContentType != null)
                return supportedContentType;

            if (string.IsNullOrWhiteSpace(requestContentType) || requestContentType.Contains(ContentType.PlainText)
                || requestContentType.Contains(ContentType.Html))
            {
                if (responseContentType != null)
                    return responseContentType;

                var specifiedContentType = GetQueryStringContentType(httpReq);
                if (!string.IsNullOrWhiteSpace(specifiedContentType))
                    return specifiedContentType;

                if (httpReq.AcceptTypes != null && httpReq.AcceptTypes.Length == 1)
                {
                    supportedContentType = GetSupportedContentType(httpReq.AcceptTypes[0]);
                    if (supportedContentType != null)
                        return supportedContentType;
                }

                if (EndpointHost.Config.DefaultContentType != null)
                    return EndpointHost.Config.DefaultContentType;

                if (httpReq.AcceptTypes != null && httpReq.AcceptTypes.Length > 1)
                {
                    foreach (string contentType in httpReq.AcceptTypes)
                    {
                        supportedContentType = GetSupportedContentType(contentType);
                        if (supportedContentType != null)
                            return supportedContentType;
                    }
                }
        ...
        public static string GetResponseContentType(this IHttpRequest httpReq)
        {
            ...
            if (httpReq.AcceptTypes != null && httpReq.AcceptTypes.Length == 1)
            {
                string supportedContentType = GetSupportedContentType(httpReq.AcceptTypes[0]);
                if (supportedContentType != null)
                    return supportedContentType;
            }

            string requestContentType = httpReq.GetSupportedContentType();
            ...
            if (httpReq.AcceptTypes != null && httpReq.AcceptTypes.Length > 1)
            {
                foreach (string contentType in httpReq.AcceptTypes)
                {
                    string supportedContentType = GetSupportedContentType(contentType);
                    if (supportedContentType != null)
                        return supportedContentType;
                }
            }
        ...
        {
            string baseUrl = EndpointHost.Config.WebHostUrl;
            if (baseUrl == null)
            {
                if (httpReq.OriginalRequest is HttpRequest)
                {
                    HttpRequest request = httpReq.OriginalRequest as HttpRequest;
                    baseUrl = request.GetApplicationUrl();
                }
                else
                    baseUrl = httpReq.GetPathUrl();
            }
            ...
            if (handlerPath != null)
            {
                ...
                if (pos >= 0)
                {
                    return httpReq.AbsoluteUri.Substring(0, pos + handlerPath.Length);
                }
                return "/" + handlerPath;
            }
