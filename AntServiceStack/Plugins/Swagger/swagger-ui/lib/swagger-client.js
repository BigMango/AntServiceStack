  ...
  var obj = {
    ...
    on: {
      ...
      response: function(resp) {
        ...
        if (responseObj.apis.length < 1) {
            $(".loading").hide();
            return self.fail('auth error ' + self.url);
        }
...
Model.prototype.createJSONSample = function(modelsToIgnore) {
  ...
  for (i = 0; i < this.properties.length; i++) {
    ...
    var description = prop.description;
      console.log(prop);
    result[prop.name + (description?"["+description +"]":"")] = sample;
  }
...
Model.prototype.getSampleValue = function(modelsToIgnore) {
  ...
  for(i = 0; i < this.properties.length; i++ ) {
    ...
    obj[property.name+ (description?"["+description +"]":"")] = property.sampleValue(false, modelsToIgnore, representations);
  }
  return obj;
};
...
SwaggerModel.prototype.createJSONSample = function (modelsToIgnore) {
  if (sampleModels[this.name]) {
    ...
    for (var i = 0; i < this.properties.length; i++) {
      ...
      var description = prop.descr;
      result[prop.name + (description?"["+description +"]":"")] = prop.getSampleValue(modelsToIgnore);
    }
...
SwaggerOperation.prototype.supportHeaderParams = function () {
  return true;//this.resource.api.supportHeaderParams;
};
