...
namespace AntServiceStack.CacheAccess.Providers
{
        ...
        : ICacheClient, IRemoveByPattern
    {
        ...
        public void RemoveAll(IEnumerable<string> keys)
        {
            foreach (var key in keys)
            {
                ...
                {
                    Log.Error(string.Format("Error trying to remove {0} from the cache", key), ex, new Dictionary<string, string>() { { "ErrorCode", "FXD300040" } });
                }
            }
        }
        ...
        public void RemoveByRegex(string pattern)
        {
            ...
            {
                Log.Error(string.Format("Error trying to remove items from cache with this {0} pattern", pattern), ex, new Dictionary<string, string>() { { "ErrorCode", "FXD300041" } });
            }
        }
    }
}