...
namespace AntServiceStack.Common.Consul
{
    ...
    public static class ConsulClient
    {
        ...
        public static void RegisterService(ServiceRegistration registration)
        {
            ...
            var putUrl = "/v1/agent/service/register";
            var registrationUrl = ConsulUris.LocalAgent.CombineWith(putUrl);
            registrationUrl.PostJsonToUrl(consulServiceRegistration, null,
                response =>
                {
                    ...
                    {
                        logger.Fatal("Could not register appHost with Consul. It will not be discoverable: {0}".Fmt(consulServiceRegistration));
        ...
        public static void RegisterHealthChecks(params ServiceHealthCheck[] healthChecks)
        {
            ...
            foreach (var check in healthChecks)
            {
                try
                {
                    ...
                    var registerUrl = "/v1/agent/check/register";
                    ConsulUris.LocalAgent.CombineWith(registerUrl).PutJsonToUrl(
                        ...
                        response =>
                        {
                            ...
                            {
                                logger.Error(
                                    "Could not register health check ${check.Id} with Consul. {0}".Fmt(response.StatusDescription));
                            }
                            else
                            {
                                logger.Info("Registered health check with Consul `{0}`".Fmt(check.Id));
                            }
                        });
                }
                catch (Exception ex)
                {
                    ...
                    throw new GatewayServiceDiscoveryException("Could not register service health check {0}".Fmt(check.Id), ex);
                }
            }
        }
