...
namespace AntServiceStack.Common.Configuration
{
	public class ConfigUtils
	{
	    ...
	    {
            // key case insensitive dictionary
            ...
            appSettingValue = appSettingValue.Trim();
            foreach (var item in appSettingValue.Split(new char[] { ItemSeperator }, StringSplitOptions.RemoveEmptyEntries))
	        {
                var keyValuePair = item.Split(new char[] { KeyValueSeperator }, StringSplitOptions.RemoveEmptyEntries);
                if (keyValuePair == null || keyValuePair.Length != 2 || string.IsNullOrWhiteSpace(keyValuePair[KeyIndex]))
                    continue;
	            dictionary[keyValuePair[KeyIndex].Trim()] = keyValuePair[ValueIndex].Trim();
	        }
	        return dictionary;
	    }
