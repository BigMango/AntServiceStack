    ...
    public sealed class ServiceDescriptionEngine
    {
            ...
            string wsdlSaveLocation, string xmlComment, string wsdlLocation)
        {
            ...
            if (File.Exists(fileName))
            {
                ...
                {
                    throw new WsdlGenerationException("An error occured while trying to generate a WSDL. Failed to backup the existing WSDL file. " + ex, ex);
                }
            }

            StreamWriter writer1 = new StreamWriter(fileName, false, Encoding.UTF8);
            try
            {
        ...
        public static InterfaceContract GetInterfaceContract(string wsdlFileName)
        {
            ...
            ValidateWsdl(srvDesc, ref isHttpBinding);
            ...
            foreach (FxOperation op in ptype.Operations)
            {
                ...
                if (op.Faults != null && op.Faults.Count > 0)
                {
                    throw new WsdlNotCompatibleException("Soap fault defined in operation " + op.Name + " is not supported.");
                }
                ...
                if (op.Messages.Input != null)
                {
                    ...
                    if (inMessage == null)
                    {
                        throw new WsdlNotCompatibleException("Could not find the input message : " + 
                            op.Messages.Input.Message.Name + " defined in opearation : " + op.Name);
                    }
                ...
                {
                    throw new WsdlNotCompatibleException("Missing input message in operation : " + op.Name);
                }
                ...
                if (op.Messages.Output != null)
                {
                    ...
                    if (outMessage == null)
                    {
                        throw new WsdlNotCompatibleException("Could not find the output message : " + 
                            op.Messages.Output.Message.Name + " defined in operation : " + op.Name);
                    }
                ...
                {
                    throw new WsdlNotCompatibleException("Missing output message in operation : " + 
                        op.Name + ", oneway style is not supported");
                }
                        ...
                        if (simpleOp.Name == opBinding.Name)
                        {
                            if (opBinding.Input != null)
                            {
                                ...
                                foreach (ServiceDescriptionFormatExtension extension in opBinding.Input.Extensions)
                                {
                                    ...
                                    if (inHeader != null)
                                    {
                                        throw new WsdlNotCompatibleException("Input Soap header binding defined in operation binding" 
                                            + opBinding.Name + " is not supported.");
                                    }
                                }
                            }
                            else
                            {
                                // WSDL modified.
                                throw new WsdlNotCompatibleException("Missing input in operation binding : " + opBinding.Name);
                            }
                            ...
                            if (opBinding.Output != null)
                            {
                                ...
                                foreach (ServiceDescriptionFormatExtension extension in opBinding.Output.Extensions)
                                {
                                    ...
                                    if (outHeader != null)
                                    {
                                        throw new WsdlNotCompatibleException("Output Soap header binding is not supported");
                                    }
                                }
                            }
                            else
                            {
                                // WSDL modified.
                                throw new WsdlNotCompatibleException("Missing output in operation binding : " 
                                    + opBinding.Name + ", oneway style is not supported");
                            }
                        }
                    }
                }
        ...
        /// <summary>
        /// Validates a specified instance of <see cref="ServiceDescription"/> class for ant SOA compatibility
        /// </summary>
        ...
        /// <exception cref="">exception thrown if the wsdl is not compatible with ant SOA standard</exception>
        private static void ValidateWsdl(
            ...
            ref bool isHttpBinding)
        {
            ...
            {
                throw new WsdlNotCompatibleException("Service description must have atleast one schema in the types definitions.");
            }

            // Rule No 2: Service description must have only one <porttype>.
            if (serviceDescription.PortTypes.Count != 1)
            {
                throw new WsdlNotCompatibleException("Service description must have only one <porttype>.");
            }
            ...
            {
                throw new WsdlNotCompatibleException("Service description must have only SOAP 1.1 and/or SOAP 1.2 binding(s).");
            }
            ...
            {
                throw new WsdlNotCompatibleException("Service description can not have more than one <service>.");
            }
            ...
            foreach (FxMessage message in serviceDescription.Messages)
            {
                ...
                {
                    throw new WsdlNotCompatibleException("Each message must have only one <part>.");
                }
            }
            ...
            foreach (ServiceDescriptionFormatExtension extension in binding.Extensions)
            {
                ...
                if (soapBinding != null)
                {
                    if (soapBinding.Style != SoapBindingStyle.Default && soapBinding.Style != SoapBindingStyle.Document)
                    {
                        throw new WsdlNotCompatibleException("For soap bindings the binding style must be 'Document' and encoding must be 'Literal'");
                    }
                }
            ...
            foreach (OperationBinding operationBinding in binding.Operations)
            {
                ...
                foreach (ServiceDescriptionFormatExtension extension in operationBinding.Extensions)
                {
                    ...
                    {
                        if (soapOperationBinding.Style != SoapBindingStyle.Default && soapOperationBinding.Style != SoapBindingStyle.Document)
                        {
                            throw new WsdlNotCompatibleException("For soap bindings the binding style must be 'Document' and encoding must be 'Literal'");
                        }
                    }
